...
void MirrorManager::Update(float deltaTime) {
	...
	if (localAddMirrorButton->IsClicked()) {
		...
		//To determine of mirror is already spawned
		bool notMirrorSpawnedYet = false;

		//For every mirror check if position is equal to spawn position
		//If so set boolean
		for (auto it = mirrors.begin(); it != mirrors.end(); it++) {
			if (it->second->GetComponent<BC::Sprite>()->getPosition() == sf::Vector2f(430, 100)) {
				notMirrorSpawnedYet = true;
			}
		}

		//If no mirror is on spawn position then continue
		if (!notMirrorSpawnedYet) {
			//Play click sound
	...
	if (reflectorBeamManager->GetBeams().size() > 0) {
		...
		for (auto ITBEAMS = reflectorBeamManager->GetBeams().begin(); ITBEAMS != reflectorBeamManager->GetBeams().end(); ITBEAMS++) {
			...
			if (ITBEAMS->second != nullptr) {
				...
				GameObject* closestMirror = GetClosestMirrorForBeam(*beam);

				//If size is bigger then 0
				if (mirrors.size() > 0) {
					//If there is a closest mirror
					if (closestMirror) {
						//Get shortest hit position between the beam and mirror
						std::string shortestDistance = GetClosestHitLocationInClosestMirrorForBeam(*beam, *closestMirror);
						sf::Vector2f colliderSide = GetSideHitpointInClosestMirrorForBeamByName(shortestDistance, *beam, *closestMirror);

						//If hit distances map size is bigger then 0
						if (colliderSide != sf::Vector2f(-100, -100)) {
							//Set variables
							CollidesWithAMirror = true;
							beam->reflectingMirror = closestMirror;
							line[1].position = colliderSide;
							...
							//If the shortest distance is the front
							if (shortestDistance == "Front")  {
								//Get the angle between a beam and mirror
								float AngleBetweenBeamAndMirror = GetAngleBetweenMirrorAndIncomingBeam(*beam, *closestMirror);
								//Get rotation for the new beam
								float newBeamRotation = GetRotationForNewBeam(*beam, *closestMirror, AngleBetweenBeamAndMirror);

								//If mirror is not a static object
								if (!closestMirror->GetComponent<Mirror>()->IsStaticObject()) {
									//If new beam of beam is not yet made
									if (!beam->newBeam) {
										//Create beam
										GameObject* go = &reflectorBeamManager->AddBeam(colliderSide, newBeamRotation, line[0].color);
									...
									//If beam is already created
										...
										sf::Vector2f newBeamDirection = Equations::CreateDirectionFromRotation(newBeamRotation);
										
										beam->newBeam->GetComponent<ReflectorBeam>()->SetDirection(newBeamDirection);
									}
								}
								else {
									//If new beam excist desroy it
					...
					//If reflecting mirror of beam is nullptr or mirror map size is 0
...
Mirror& MirrorManager::AddMirror(sf::Vector2f position, float rotation) {
	...
	BC::Sprite* mirrorSprite = new BC::Sprite("../Assets/Art/Level/Mirror.png");
	...
	//Set standard values of mirrorSprite
...
//Returns the closest mirror to a beam
GameObject* MirrorManager::GetClosestMirrorForBeam(ReflectorBeam& beam) {
	...
	//Return the closest mirror variable
	return closestMirror;
}

//Returns closest hit location inside the closest mirror to a beam
std::string MirrorManager::GetClosestHitLocationInClosestMirrorForBeam(ReflectorBeam& beam, GameObject& closestMirror) {
	//Create necessary local variables
	std::string tempShortestDist = "Empty";
	std::map<std::string, float> pointsHitDistances;

	sf::Vertex* line = beam.GetLine();

	//Create the line detection vectors
	sf::Vector2f frontsideMirrorCollider = GetSideHitpointInClosestMirrorForBeamByName("Front", beam, closestMirror);
	sf::Vector2f backsideMirrorCollider = GetSideHitpointInClosestMirrorForBeamByName("Back", beam, closestMirror);
	sf::Vector2f rightsideMirrorCollider = GetSideHitpointInClosestMirrorForBeamByName("Right", beam, closestMirror);
	sf::Vector2f leftsideMirrorCollider = GetSideHitpointInClosestMirrorForBeamByName("Left", beam, closestMirror);
	...
	tempShortestDist = shortestDist->first;

	//Return the shortest distance side as a string
	return tempShortestDist;
}

//Calculates a hit point by a specified side
sf::Vector2f MirrorManager::GetSideHitpointInClosestMirrorForBeamByName(std::string sideName, ReflectorBeam& beam, GameObject& closestMirror) {
	//Create necessary local variables
	sf::Vertex* line = beam.GetLine();
	...
	//Depending on the sideName, return a different Equation
	if (sideName == "Front") {
		return Equations::LineCollide(line[0].position, line[1].position, beam.GetDirection(), p0, p1, *closestMirror.GetComponent<BC::Sprite>());
	}
	else if (sideName == "Back") {
		return Equations::LineCollide(line[0].position, line[1].position, beam.GetDirection(), p3, p2, *closestMirror.GetComponent<BC::Sprite>());
	}
	else if (sideName == "Left") {
		return Equations::LineCollide(line[0].position, line[1].position, beam.GetDirection(), p0, p3, *closestMirror.GetComponent<BC::Sprite>());
	}
	else if (sideName == "Right") {
		return Equations::LineCollide(line[0].position, line[1].position, beam.GetDirection(), p1, p2, *closestMirror.GetComponent<BC::Sprite>());
	}
}

//Gets the angle between a mirror and a beam
float MirrorManager::GetAngleBetweenMirrorAndIncomingBeam(ReflectorBeam& beam, GameObject& closestMirror) {
	//Create necessary local variables
	float radians;
...
float MirrorManager::GetRotationForNewBeam(ReflectorBeam& beam, GameObject& closestMirror, float angleBetweenBeamAndMirror) {
	//Create necessary local variables
	float rotation;

	//Calculate new direction of beam 
	sf::Vector2f dir = Equations::CreateDirectionFromRotation(closestMirror.GetComponent<BC::Sprite>()->getRotation() + angleBetweenBeamAndMirror);
	...
	float newBeamX = roundf(dir.x * 100) / 100;
	float newBeamY = roundf(dir.y * 100) / 100;
	...
	if (newBeamX == beamX && newBeamY == beamY) {
		rotation = closestMirror.GetComponent<BC::Sprite>()->getRotation() - angleBetweenBeamAndMirror;
	}
	else {
		rotation = closestMirror.GetComponent<BC::Sprite>()->getRotation() + angleBetweenBeamAndMirror;
	}
